...
class GridWorm {
  ...
  update(deltaTime) {
    ...
    if (
      this.xCoord >=
      this.screenWidth - this.interval
    ...
    if (
      this.yCoord >=
      this.screenHeight - this.interval
    ...
    if (latestJunction !== currentCoord) {
      ...
      if (originalVelocity.y === 0) //if gridworm is moving horizontally
      {
        ...
        if (
          newVelocity.y === 0 &&
          newVelocity.x === -originalVelocity.x
    ...
    if (
      this.junctionMemory.length > this.junctionMemoryLength
  ...
  isInMemory(memory) {
    //check if a junction is in memory
